...
export async function run() {
    ...
    try {
        ...
        const experimentalLocalizeStaticUrls = core.getBooleanInput('experimental_localize_static_urls');
        const experimentalHideDefaultLocale = core.getBooleanInput('experimental_hide_default_locale');
        const experimentalFlattenJsonFiles = core.getBooleanInput('experimental_flatten_json_files');
        ...
        if (dictionary)
            args.push('--dictionary', dictionary);
        if (src)
            args.push('--src', src);
        if (defaultLanguage)
            args.push('--default-language', defaultLanguage);
        if (locales)
...
async function createPR(githubToken, prBranch, prTitle, prBody) {
    ...
    if (prAlreadyExists) {
        core.info(`PR already exists for branch ${availableBranchName}, skipping PR creation`);
        return;
    }
    // Create PR
